...
def _load_site_info(site_info_file: Path) -> dict[str, dict]:
    ...
    for _, row in df.iterrows():
        # 设备号在列索引 5，点位名称在列索引 6
        point_name_full = str(row.iloc[6]) if pd.notna(row.iloc[6]) else ""
        ...
        diameter = float(row.iloc[11]) if pd.notna(row.iloc[11]) else 0.0
        depth = float(row.iloc[12]) if pd.notna(row.iloc[12]) else 0.0
        pipe_type = str(row.iloc[13]) if pd.notna(row.iloc[13]) else ""
