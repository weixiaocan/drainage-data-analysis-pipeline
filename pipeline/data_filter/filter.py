...
def _load_rainfall_daily(rainfall_file: Path) -> pd.Series:
    """加载降雨数据，按日汇总"""
    rain = _read_csv_with_fallback(rainfall_file)
    time_col = "t" if "t" in rain.columns else next(c for c in rain.columns if "time" in str(c).lower() or "时间" in str(c))
    rain_col = "rain" if "rain" in rain.columns else next(c for c in rain.columns if "rain" in str(c).lower() or "雨" in str(c))
